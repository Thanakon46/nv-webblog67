<template>
  <div>
    <h1>Create Cap</h1>
    <form v-on:submit.prevent="createCap">
      <p>
        title:
        <input type="text" v-model="cap.title" />
      </p>
      ...
      <p>
        category:
        <input type="text" v-model="cap.category" />
      </p>
      <p>
        status:
        <input type="text" v-model="cap.status" />
      </p>
      <p>
        <button type="submit">create cap</button>
      </p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async createCap() {
      ...
      console.log("JSON.stringify: ", this.cap);
      try {
        await CapsService.post(this.cap);
        this.$router.push({
          name: "caps",
        });
      } catch (err) {
